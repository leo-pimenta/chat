...
namespace Test
{
    public class KafkaProducerTest
    {
        ...
        {
            var message1 = new Message("key1", "body1");
            var message2 = new Message("key2", "body2");
            var message3 = new Message("key3", "body3");
            
            Service.Add(message1, message1.Key);
            Service.Add(message2, message2.Key);
            Service.Add(message3, message3.Key);
        ...
        public void Should_SetCorrectTimestamp_InNewMessage()
        {
            ...
            Service.Add(new Message(key, "body1"), key);
        ...
        {
            const string key = "key1";
            var message = new Message("key1", "body1");
            Service.Add(message, key);
            MockMessageStore.Should().HaveCount(1);
            MockMessageStore[0].Key.Should().BeEquivalentTo(message.Key);
        }
        ...
        public void ShouldThrow_IfAdd_HasInvalidInput()
        {
            ...
            Assert.Throws<ArgumentException>(() => Service.Add(new Message("a", "a"), null!));
            Assert.Throws<ArgumentException>(() => Service.Add(new Message("a", "a"), ""));
            Assert.Throws<ArgumentException>(() => Service.Add(new Message("a", "a"), "  "));
        }
